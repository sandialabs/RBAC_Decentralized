...
<div class="yourDivClass">
  ...
  <br>
  <br>
    <h1 style='font-size:45px;'>Service Provider 6</h1>
     <br>
...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/sp6';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/sp6/${entityID}`;
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/sp6revoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
