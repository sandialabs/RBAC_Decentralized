...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/secauditors';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/secauditors/${entityID}`;
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/secauditorsrevoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
