...
<div class="yourDivClass">
  ...
  <br>
  <br>
    <h1 style='font-size:45px;'>DER Owners</h1>
     <br>
...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/derowners';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/derowners/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/derownersrevoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
