...
<div class="yourDivClass">
  ...
  <br>
  <br>
    <h1 style='font-size:45px;'>Utility 3</h1>
     <br>
...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/utility3';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/utilities3/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/utilities3revoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
