...
<div class="yourDivClass">
  ...
  <br>
  <br>
    <h1 style='font-size:45px;'>Security Administrators</h1>
     <br>
...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/secadmins';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/secadmins/${entityID}`;
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/secadminsrevoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
