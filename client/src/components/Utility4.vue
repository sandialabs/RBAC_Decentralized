...
<div class="yourDivClass">
  ...
  <br>
  <br>
    <h1 style='font-size:45px;'>Utility 4</h1>
     <br>
...
export default {
  ...
  methods: {
    ...
    getEntities() {
      const path = 'http://localhost:5000/utility4';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload, entityID) {
      const path = `http://localhost:5000/utilities4/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
    ...
    revokeRole(payload, entityID) {
      const path = `http://localhost:5000/utilities4revoke/${entityID}`;
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
