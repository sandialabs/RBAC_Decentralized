...
# ----- Create a dictionary that stores all the permissions (keys: Roles, Values:Perms)----- #
perm_dict = {}
for r in rbac_roles:
    perm_dict[r] = {}
...
for r in rbac_roles:
    ...
    print_permissions_for_role = True
    if print_permissions_for_role:
        ...
        # ------ Parsing the inv object for every Model | Taking the R/W Perms from the points var ------
        for temp_model, perm_list in inv.models.items():
            if (not isinstance(temp_model,int)):
                perm_dict[r][temp_model] = []
                temp = {}
                print('Model: ' + str(temp_model))
                #if (not isinstance(modell,int)):
                for perm, temp_list in inv.models[temp_model][0].points.items():
                    rbac_list = inv.models[temp_model][0].points[perm].pdef['rbac']
                    temp[perm] = rbac_list[r]
                perm_dict[r][temp_model].append(temp)
        # --------------------------------------------------------------------------------------------------
